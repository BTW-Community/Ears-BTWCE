...
import com.unascribed.ears.api.Identified;
...
import com.unascribed.ears.main.debug.DebuggingDelegate;
import com.unascribed.ears.main.debug.EarsLog;
import com.unascribed.ears.main.render.EarsRenderDelegate;

import com.unascribed.ears.main.render.IndirectEarsRenderDelegate;
...
class EarsRenderer {
	...
	private static void renderInner(EarsFeatures features, EarsRenderDelegate delegate, int p, boolean drawingEmissive) {
		...
		if (features != null && features.enabled) {
			...
			if (chestSize > 0 &&
					(!isActive(delegate, EarsStateType.WEARING_CHESTPLATE) || delegate.canBind(EarsRenderDelegate.TexSource.CHESTPLATE)) &&
					(
							p == 0 ||
							(p == 1 && delegate.isJacketEnabled()) ||
							((p == 2 || (p == 3 && delegate.canBind(EarsRenderDelegate.TexSource.GLINT_CHESTPLATE))) &&
									isActive(delegate, EarsStateType.WEARING_CHESTPLATE) && !drawingEmissive && delegate.canBind(EarsRenderDelegate.TexSource.CHESTPLATE)))
